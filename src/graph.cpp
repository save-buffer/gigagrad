...
namespace gigagrad
{
    ...
    std::string getCDatatype(NumericDataType dtype)
    {
        switch (dtype)
        {
        case NumericDataType::FLOAT64:
            return "double";
        case NumericDataType::FLOAT32:
            return "float";
        case NumericDataType::FLOAT16:
            return "__fp16"; // Check compiler support for this type.
        case NumericDataType::INT8:
            return "int8_t";
        case NumericDataType::INT16:
            return "int16_t";
    ...
    static NumericDataType CompareDtypes(const NumericDataType &dtype_x, const NumericDataType &dtype_y)
    {
        auto precedence = [](const NumericDataType &dtype)
        {
            switch (dtype)
            {
            case NumericDataType::FLOAT64:
                return 8; // Highest precedence
            case NumericDataType::FLOAT32:
                return 7;
            case NumericDataType::FLOAT16:
                return 6; // Higher than INT64 despite its smaller size
            case NumericDataType::INT64:
                return 5;
            case NumericDataType::INT32:
                return 4;
        ...
        // Compare precedences
        if (precedence(dtype_x) >= precedence(dtype_y))
        {
            return dtype_x;
        }
        else
    ...
    GraphNodeHandle operator+(double x, GraphNodeHandle y)
    {
        Graph *graph = y.graph;
    ...
    GraphNodeHandle operator+(GraphNodeHandle x, double y)
    {
        return y + x;
    }
    ...
    GraphNodeHandle operator==(const GraphNodeHandle x, double y)
    {
        return y == x;
    }
    ...
    GraphNodeHandle operator<(double x, const GraphNodeHandle y)
    {
        return y > x;
    }
    ...
    GraphNodeHandle max(double x, GraphNodeHandle y)
    {
        Graph *graph = y.graph;
        GraphNodeHandle xnode = graph->AddNode(Immediate{x});
        return max(xnode, y);
    ...
    GraphNodeHandle max(GraphNodeHandle x, double y)
    {
        return max(y, x);
    }
    ...
    GraphNodeHandle min(double x, GraphNodeHandle y)
    {
        return -max(-x, -y);
    }

    GraphNodeHandle min(GraphNodeHandle x, double y)
    {
        return -max(-x, -y);
    }

    GraphNodeHandle pow(GraphNodeHandle x, double y)
    {
        Graph *graph = x.graph;
        GraphNodeHandle ynode = graph->AddNode(Immediate{y});
        return graph->AddNode(BinaryOp{BinaryOpType::POW, x, ynode});
    ...
    GraphNodeHandle pow(double x, GraphNodeHandle y)
    {
        Graph *graph = y.graph;
        GraphNodeHandle xnode = graph->AddNode(Immediate{x});
        // NumericDataType numeric_data_type = xnode->numeric_data_type
    ...
    GraphNodeHandle Graph::Immediate(double imm)
    {
        return this->AddNode(gigagrad::Immediate{imm});
    }

    GraphNodeHandle Graph::AddInput(Shape shape, NumericDataType dtype)
    {
        this->inputs.push_back(this->nodes.size());
        GraphNodeHandle result = this->AddNode(Tensor{}, std::move(shape), std::move(dtype));
        return result;
    }

    GraphNodeHandle Graph::AddInput(dim_t dim, NumericDataType dtype)
    {
        return this->AddInput(Shape{dim}, NumericDataType::FLOAT32);
    }

    GraphNodeHandle Graph::AddNode(Tensor tensor, Shape shape, NumericDataType dtype)
    {
        Shape strides = ComputeStrides(shape);
        return this->AddNode(
            GraphNode{
    ...
    GraphNodeHandle Graph::AddNode(BinaryOp op)
    {
        ...
        Shape strides = ComputeStrides(shape);
        NumericDataType dtype = CompareDtypes(op.x.dtype(), op.y.dtype());
        return this->AddNode(
            GraphNode{
                .u = {std::move(op)},
                .shape = std::move(shape),
    ...
    const NumericDataType &GraphNodeHandle::dtype() const
    {
        const GraphNode &node = this->GetNode();
        return node.dtype;
    }
    ...
    GraphNodeHandle nn::Module::Immediate(double imm)
    {
        return this->graph.Immediate(imm);
    }

    GraphNodeHandle nn::Module::AddInput(Shape shape, NumericDataType dtype)
    {
        return this->graph.AddInput(std::move(shape), std::move(dtype));
    }

    GraphNodeHandle nn::Module::AddInput(dim_t dim, NumericDataType dtype)
    {
        return this->graph.AddInput(dim, std::move(dtype));
    }

    GraphNodeHandle nn::Module::AddWeight(Shape shape, NumericDataType dtype)
    {
        this->weights.push_back(this->graph.inputs.size());
        return this->graph.AddInput(std::move(shape), std::move(dtype));
    }

    GraphNodeHandle nn::Module::AddWeight(dim_t dim, NumericDataType dtype)
    {
        this->weights.push_back(this->graph.inputs.size());
        return this->graph.AddInput(dim, std::move(dtype));
    }
