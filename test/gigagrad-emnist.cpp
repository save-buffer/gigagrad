...
template <typename T>
std::vector<double> CastToFloatAndNormalize(const std::vector<uint8_t> &input)
{
    ...
    {
        result[i] = static_cast<double>(input_data[i]) / 255.0;
    }
    return result;
}

std::vector<double> CastToFloatAndNormalize(DataType dtype, const std::vector<uint8_t> &input)
{
    switch (dtype)
    {
    case DataType::U8:
...
template <typename T>
std::vector<double> ToOneHot(const std::vector<uint8_t> &input)
{
    size_t num_elements = input.size() / sizeof(T);
    const T *input_data = reinterpret_cast<const T *>(input.data());
    T max_val = 0;
    ...
    std::vector<double> result(max_val * num_elements, 0.0f);
    for (size_t i = 0; i < num_elements; i++)
    {
...
std::vector<double> ToOneHot(DataType dtype, const std::vector<uint8_t> &input)
{
    switch (dtype)
    {
    case DataType::U8:
...
Dataset LoadDataset(const char *directory, const char *dataset)
{
    ...
    std::vector<double> images_float = CastToFloatAndNormalize(images.dtype, images.data);
    std::vector<double> labels_onehot = ToOneHot(labels.dtype, labels.data);
    return {std::move(images.shape), std::move(images_float), std::move(labels_onehot)};
}

void InitializeWeights(double *weight, size_t size_elts)
{
...
int main(int argc, const char **argv)
{
    ...
    w1.data() = new double[HiddenLayerSize * 28 * 28];
    b1.data() = new double[HiddenLayerSize * 1];
    w2.data() = new double[10 * HiddenLayerSize];
    b2.data() = new double[10 * 1];
